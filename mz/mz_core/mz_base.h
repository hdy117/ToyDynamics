...
#define MzSharedPtr boost::shared_ptr
#define MakeMzSharedPtr boost::make_shared
#else
#include <memory>

#define MzSharedPtr std::shared_ptr
#define MakeMzSharedPtr std::make_shared
#endif // USE_STD_SHARED_PTR

namespace mz {
template <typename TyPtr> bool isNullPtr(TyPtr &&ptr) {
  return ptr.get() == nullptr;
}

template <typename TyPtr> bool isNotNullPtr(TyPtr &&ptr) {
  return ptr.get() != nullptr;
}
...
using IntegratorVector3Ptr = SIM_API MzSharedPtr<IntegratorVector3>;

// global integrator
SIM_API void setGlobalIntergator(
    const INTEGRATOR_TYPE &integratorType = INTEGRATOR_TYPE::FORWARD_EULER);
...
public:
  static void debugPrintVector3(const std::string &prefix,
