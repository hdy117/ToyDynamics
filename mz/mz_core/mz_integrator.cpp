...
void EulerIntegrator::integrate(const Vector3& u, Vector3& y, const real deltaTime) {
  y = m_preState + u * deltaTime;
  m_preState = y;
}
void RK4Integrator::integrate(const Vector3& u, Vector3& y, const real deltaTime) {
  // @dongyuanhu
  throw std::runtime_error("RK4 integrator not implemented yet.");
}

IntegratorVector3Ptr IntegratorVector3_Maker::makeIntegrator(const INTEGRATOR_TYPE& type) {
  IntegratorVector3Ptr integrator;
  if (type == INTEGRATOR_TYPE::FORWARD_EULER) {
    return MakeSharedPtr<EulerIntegrator>();
  }
  else if (type == INTEGRATOR_TYPE::RK4) {
    return MakeSharedPtr<RK4Integrator>();
  }
  else {
